...
implements ActionListener{
	...
	public void paint(Graphics g) {
		...
		switch (idx) {
		case SHOW_NORM:
			g.drawImage(img, 0, 0, iw, ih,	//컨테이너 좌표(4개)
					0, 0, iw, ih, this);	//이미지 좌표(4개)
			break;
		case SHOW_PART:
			g.drawImage(img, 100, 100, 300, 300,	//컨테이너 좌표(4개)
					220, 20, 420, 220, this);	//이미지 좌표(4개)
			break;
		case SHOW_SCALE:
			g.drawImage(img, 0, 0, iw/2, ih/2,	//컨테이너 좌표(4개)
					0, 0, iw, ih, this);	//이미지 좌표(4개)
			break;
		case SHOW_HORIZONTAL:
			g.drawImage(img, iw, 0, 0, ih,	//컨테이너 좌표(4개)
					0, 0, iw, ih, this);	//이미지 좌표(4개)
			break;
		case SHOW_VERTICAL:
			g.drawImage(img, 0, ih, iw, 0,	//컨테이너 좌표(4개)
					0, 0, iw, ih, this);	//이미지 좌표(4개)
			break;
		case SHOW_ALL:
			g.drawImage(img, iw, ih, 0, 0,	//컨테이너 좌표(4개)
					0, 0, iw, ih, this);	//이미지 좌표(4개)
			break;
		}
	}	//paint
